...
class ExtractorsTest extends FunSpec with ShouldMatchers {
  ...
  describe("Extractors") {
    ...
    describe("string.required") {
      ...
      it("invalid") {
        Extractors.string.required.int('id) <--? "" shouldBe ExtractionFailed('id -> "invalid int")
        Extractors.string.required.long('id) <--? "" shouldBe ExtractionFailed('id -> "invalid long")
        Extractors.string.required.double('id) <--? "" shouldBe ExtractionFailed('id -> "invalid double")
        Extractors.string.required.float('id) <--? "" shouldBe ExtractionFailed('id -> "invalid float")
        Extractors.string.required.boolean('id) <--? "" shouldBe ExtractionFailed('id -> "invalid boolean")
        Extractors.string.required.char('id) <--? "" shouldBe ExtractionFailed('id -> "invalid char")
        Extractors.string.required.localDate('id) <--? "" shouldBe ExtractionFailed('id -> "invalid localDate")
        Extractors.string.required.localDateTime('id) <--? "" shouldBe ExtractionFailed('id -> "invalid localDateTime")
        Extractors.string.required.zonedDateTime('id) <--? "" shouldBe ExtractionFailed('id -> "invalid zonedDateTime")
        Extractors.string.required.bigDecimal('id) <--? "" shouldBe ExtractionFailed('id -> "invalid bigDecimal")
        Extractors.string.required.uuid('id) <--? "" shouldBe ExtractionFailed('id -> "invalid uuid")
      }
    }
